...
function DrawerContent({ className, children, ...props }: React.ComponentProps<typeof Dialog.Content>) {
  // Simple right-side sheet implementation using Radix Dialog.
  return (
    <Dialog.Portal>
      <DrawerOverlay />
      <Dialog.Content
        className={cn(
          'group/drawer-content bg-background fixed z-50 flex h-auto flex-col right-0 inset-y-0 w-3/4 border-l sm:max-w-sm',
          className,
        )}
        {...props}
      >
        <div className="bg-muted mx-auto mt-4 hidden h-2 w-[100px] shrink-0 rounded-full group-data-[state=open]/drawer-content:block" />
        {children}
      </Dialog.Content>
    </Dialog.Portal>
  )
